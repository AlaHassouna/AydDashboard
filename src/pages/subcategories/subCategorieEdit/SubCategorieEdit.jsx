    ...
    useEffect(() => {
      const fetchCategory = async () => {
        try {
          ...
          setSubCategoryName(response.data.Name); // Assurez-vous que le champ 'name' correspond à vos données
  ...
  const handleCategoryChange = (e) => {
    ...
    const handleSubmit = async (e) => {
      ...
      try {
        ...
        await axios.put(`http://127.0.0.1:8000/api/subcategories/${id}`, updatedSubCategory, {
          headers: {
            'Content-Type': 'application/json',
          },
        });
    
        setSuccess('SubCategory mise à jour avec succès.');
        setTimeout(() => navigate('/dashboard/subcategories'), 2000);
      } catch (error) {
        console.error('Erreur lors de la mise à jour de la SubCategory :', error);
        setError('Impossible de mettre à jour la SubCategory.');
      }
    };
...
export default SubCategorieEdit